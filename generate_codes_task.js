...
function generateCodes(count) {
  ...
  for (let i = 0; i < count; i++) {
    // 30 random characters (base64url is safe and compact)
    const code = crypto.randomBytes(22).toString('base64url').substring(0, 30);
    codes.push(code);
  }
  return codes;
}
...
fs.writeFileSync('codigos_regalo.txt', codes.join('\n'));

console.log('✅ 2000 codes generated in codigos_regalo.txt');

// Generate upload script for Firestore (Node.js)
const uploadScript = `
...
fs.writeFileSync('subir_codigos.js', uploadScript);
